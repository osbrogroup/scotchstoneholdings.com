...
export default function OpportunityForm() {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      fetch(`http://localhost:4000/api/opportunities/${params.id}`)
        .then(res => res.json())
        .then(data => {
  ...
  function handleSave(e: React.FormEvent) {
    ...
    const endpoint = isEdit
      ? `http://localhost:4000/api/opportunities/${params.id}`
      : `http://localhost:4000/api/opportunities/${item.title.toLowerCase().replace(/\s+/g, "-")}`;
    fetch(endpoint, {
      method: "POST",
      ...
      .then(data => {
        if (data.success) {
          alert(isEdit ? "Opportunity updated!" : "Opportunity added!");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-white to-slate-200 p-8">
      ...
      <form onSubmit={handleSave} className="bg-white rounded-xl shadow-lg p-8 space-y-6 max-w-xl mx-auto border border-gray-300">
        ...
        <div className="flex gap-2">
          ...
          >
            {isEdit ? "Save Changes" : "Save Opportunity"}
          </button>
        </div>
      </form>
      {preview && (
